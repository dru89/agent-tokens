...
# Parser registry
#
# To add a new agent, write a function with this signature:
#
#   def parse_myagent(writer: csv.DictWriter) -> int:
#       """Return the number of rows written, or 0 if the agent isn't installed."""
#
# The function receives a csv.DictWriter configured with FIELDS (see top of
# file). Each row must be a dict with these keys:
#
#   agent         — short name for the agent (e.g. "myagent")
#   timestamp     — ISO 8601 timestamp (UTC preferred)
#   input_tokens  — non-cached input tokens
#   output_tokens — tokens generated by the model
#   cache_read    — input tokens served from prompt cache (0 if unknown)
#   cache_write   — input tokens written to prompt cache (0 if unknown)
#   total_tokens  — sum of all four token fields
#
# Rules:
#   - Return 0 (don't warn) if the agent's data directory doesn't exist.
#   - Print to stderr if data exists but can't be read (permissions, corruption).
#   - Wrap file/DB access in try/except so one broken parser can't kill the run.
#
...
def main():
    agent_names = ", ".join(name for name, _ in PARSERS)

    parser = argparse.ArgumentParser(
        description=f"Extract token usage from AI coding agents ({agent_names}).",
    ...
    if total == 0:
        print(
            ...
            f"({agent_names}) has session data on this machine.",
            file=sys.stderr,
        )
    else:
